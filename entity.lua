...
M.new = function(arg)
    ...
    local collide = function(map, x1, y1, x2, y2)
        local i1 = map.getTileIndices({
            x = x1,
            y = y1
        })
        local i2 = map.getTileIndices({
            x = x2,
            y = y2
        })
        for x = i1.x, i2.x do
            for y = i1.y, i2.y do
                if map.getTileProperties({
                    x = x,
                    y = y
                }).solid == true then
                    return true
            ...
            if map then
                ...
                if self.velocity.y >= 0 then
                    if collide(map, position.x + 1, position.y + height, position.x + width, position.y + height) then
                        self.position.y = (map.getTileBounds(map.getTileIndices({
                            x = position.x,
                            y = position.y + height
                        })).top - height)
                        -- if self.scaled then
                ...
                if self.velocity.x >= 0 then
                    if collide(map, position.x + width, position.y, position.x + width, position.y + height - 1) then
                        self.position.x = (map.getTileBounds(map.getTileIndices({
                            x = position.x + width,
                            y = position.y
                        })).left - width)
                        -- if self.scaled then
    ...
    object.position = function(arg)
        ...
        return {
            x = self.position.x * M.scale,
            y = self.position.y * M.scale
        }
    end
    object.velocity = function(arg)
        ...
        return {
            x = self.velocity.x * M.scale,
            y = self.velocity.y * M.scale
        }
    end
    object.acceleration = function(arg)
        ...
        -- if self.scaled then
        return {
            x = self.acceleration.x * M.scale,
            y = self.acceleration.y * M.scale
        }
        -- else
...
M.registerType("grower", {
    ...
    render = function(object, self)
        ...
        love.graphics.draw(spritesheet, self.sprite, position.x, position.y, 0, width / 128, height / 128)
    end
})
...
M.registerType("door", {
    render = function(object, self)
        ...
        love.graphics.setColor({255, 255, 255})
        love.graphics.draw(spritesheet, sprite, position.x, position.y, 0, width, height)
    end
})
